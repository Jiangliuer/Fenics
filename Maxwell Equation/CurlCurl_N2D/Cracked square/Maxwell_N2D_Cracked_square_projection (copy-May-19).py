...
and exact solution given by
    ...
    u1 = -(1.0/2.0)*r**(-1.0/2.0)*sin((1.0/2.0)*theta)*(x**2 - 1)*(y**2 - 1) + 
         2*x*r**(1.0/2.0)*sin((1.0/1.0)*theta)*(y**2 - 1)
...
def project_boundary(x, on_boundary):
        tol=1.0e-14
        return on_boundary
#(abs(x[1]-1)<tol or abs(x[1]+1)<tol or abs(x[0]-1)<tol or abs(x[0]+1)<tol or (x[0]>tol and abs(x[1])<tol)) 
...
print "Solved!"
#(u_0,u_b,Div_R_u,Curl_R_u) = split(U_0)
(u_0,ub,Div_R_u,Curl_R_u) = U_0.split(deepcopy=True)  #deepcopy
#u0 = interpolate(u_0 + ub,FunctionSpace(mesh,"Lagrange",degree= 3))
u0 = u_0 + ub
#plot(u0[0])
#plot(u0[1])
