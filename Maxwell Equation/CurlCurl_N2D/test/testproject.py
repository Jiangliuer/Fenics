...
# Define Dirichlet boundary condition(x = 0 or x = 1 or y=0 or y=1) u=0 and (point(0,0)) p=0
def boundary(x):
...
def project_boundary(x, on_boundary):
        tol=1.0e-14
        return on_boundary and ( abs(x[1]-1)<tol or abs(x[1]+1)<tol or (abs(x[1])<tol and x[0]>tol) or  abs(x[0]-1)<tol or abs(x[0]+1)<tol or (abs(x[0])<tol and x[1]<tol))
...
L_s = inner(g,v_q)*dx
Sg = Function(Q_h)
(A_s, b_s) = assemble_system(a_s, L_s, bc_0)
